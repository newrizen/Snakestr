...
export class SnakeGame {
  ...
  draw() {
    ...
    for (let i = 0; i < this.paddleHeight / this.cellSize; i++) { // 100/ (600/60)
      this.ctx.fillText(
        CONFIG.DEFAULT_EMOJI1,
        this.leftPaddle.x,
        this.leftPaddle.y + i * (this.cellSize / 2 + 5) // spacingFactor
      );
    }
    ...
    for (let i = 0; i < this.paddleHeight / this.ballSize; i++) {
      this.ctx.fillText(
        ...
        this.rightPaddle.y + (this.ballSize / spacingFactor + 7) * (i + 2) // spacingFactor
      );
    }
